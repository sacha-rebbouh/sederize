...
import { Check, X, Layers, Palette, Folder } from 'lucide-react';
...
interface WaterfallAssignDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSelect: (selection: WaterfallSelection) => void;
  currentValue?: WaterfallSelection;
  title?: string;
}

export function WaterfallAssignDialog({
  open,
  onOpenChange,
  onSelect,
  currentValue,
  title = 'Assigner la tâche',
}: WaterfallAssignDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setSelectedCategoryId(currentValue?.categoryId || null);
      setSelectedThemeId(currentValue?.themeId || null);
      setSelectedSubjectId(currentValue?.subjectId || null);
    }
  }, [open, currentValue]);
  ...
  const handleValidate = useCallback(() => {
    ...
    onOpenChange(false);
  }, [onSelect, onOpenChange, selectedCategoryId, selectedThemeId, selectedSubjectId]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* 3 Column Layout */}
        <div className="flex-1 grid grid-cols-3 divide-x overflow-hidden" style={{ minHeight: '400px' }}>
          {/* Column 1: Categories */}
